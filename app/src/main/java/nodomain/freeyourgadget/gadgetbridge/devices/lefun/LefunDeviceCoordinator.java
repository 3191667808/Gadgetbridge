...
public class LefunDeviceCoordinator extends AbstractBLEDeviceCoordinator {
    ...
    public boolean supports(@NonNull GBDeviceCandidate candidate) {
        ...
        // Theres a bunch of other names other than "Lefun", but lets just focus on one for now.
        if (ADVERTISEMENT_NAME.equals(candidate.getName())) {
            // The device does not advertise service UUIDs, so cant check whether it supports
            // the proper service. We can check that it doesnt advertise any services, though.
            // Were actually supposed to check for presence of the string "TJDR" within the
            // manufacturer specific data, which consists of the devices MAC address and said
            // string. But were not being given it, so *shrug*.
