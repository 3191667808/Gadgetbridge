...
/**
 * Constants and helpers for the Sony Reon Tag (model RNPT-1), a small BLE beacon that is registered
 * with a Reon Pocket Pro so the Pocket can react to the tag's presence.
 *
 * The values below were reverse-engineered from a discovery capture (discover_tag.txt). During
 * discovery the phone sets a scan filter on the tag's service UUID and the tag replies with an
 * extended advertising report containing:
 *  - Complete Local Name (AD type 0x09): "RNPT-1"
 *  - Complete list of 128-bit service UUIDs (AD type 0x07):
 *      cc cf 1e 48 e1 b9 08 b3 30 43 83 e7 01 15 67 16 (little-endian on air)
 *    which reversed is the [SERVICE_UUID] below.
 */
object SonyReonTagConstants {
    ...
    /**
     * The tag's primary GATT service UUID. On air the 128-bit UUID is advertised little-endian:
     * `cc cf 1e 48 e1 b9 08 b3 30 43 83 e7 01 15 67 16`. Characteristics follow the `166715XX`
     * pattern, mirroring the Reon Pocket Pro's `04ca15XX` scheme.
     */
    ...
    // The Reon Pocket relays tag readings, so the tag is registered on the Pocket by writing its
    // Bluetooth address to the Pocket's feature characteristic (ATT handle 0x0021). Observed frames:
    //   register:   02 01 | FF EE DD CC BB AA | 01
    //   unregister: 02 02 | FF EE DD CC BB AA | 01   =  opcode | tag address (little-endian) | type.
    ...
    /** A single decoded reading relayed from a Reon Tag through the Reon Pocket Pro. */
    ...
    /**
     * Reads the tag's advertised name from the raw advertisement bytes and returns whether it
     * identifies a Reon Tag. Kept as a helper so scan-result matching stays in one place.
     */
    ...
    /**
     * Builds the payload that registers a tag with a Reon Pocket Pro, written to the Pocket's
     * feature characteristic (ATT handle 0x0021). [tagAddress] is the display-form Bluetooth address
     * (most-significant byte first, e.g. `EA:65:60:11:22:33`); it is reversed to little-endian on the
     * wire. The trailing byte is the address type (random).
     */
    ...
    /**
     * Builds the payload that unregisters a previously registered tag, written to the Pocket's
     * feature characteristic (ATT handle 0x0021). Same layout as [commandRegisterTag] but with the
     * unregister opcode (`02 02`).
     */
    ...
    /**
     * Parses a colon- or dash-separated Bluetooth address (e.g. `EA:65:60:11:22:33`) into its 6-byte
     * big-endian (display order) representation.
     */
    ...
    // --- Advertising payload (Manufacturer Specific Data, Sony companyID 0x012D) ---
    // The manufacturer payload that follows the company id is obfuscated with the fixed repeating-key
    // XOR (see [DECRYPTION_KEY]). Decrypting it exposes the tag's real identity (the on-air BD address
    // may be an ephemeral random address, so the decrypted BD address is authoritative). For the
    // decrypted buffer `dec`:
    ...
    /** The identity of a Reon Tag recovered from its (decrypted) advertising payload. */
    data class TagIdentity(
        /** Printed tag serial number (BCD-decoded, e.g. `1171234`). */
        val serial: String,
        /** Real Bluetooth device address in display form (e.g. `EA:65:60:11:22:33`). */
    ...
    /**
     * Decrypts [raw] (the obfuscated Manufacturer Specific Data payload that follows Sony's company
     * id `0x012D`) and extracts the tag's printed serial and real BD address. Returns `null` if [raw]
