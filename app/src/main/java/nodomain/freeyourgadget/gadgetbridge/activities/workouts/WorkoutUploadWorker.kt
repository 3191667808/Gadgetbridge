...
class WorkoutUploadWorker(
    ...
    override fun doWork(): Result {
        ...
        val enduDone = mutableSetOf<String>().apply {
            addAll(GBApplication.getPrefs().getStringSet(GBPrefs.ENDURAIN_UPLOADED_IDS, emptySet()))
        }
        val wandDone = mutableSetOf<String>().apply {
            addAll(GBApplication.getPrefs().getStringSet(GBPrefs.WANDERER_UPLOADED_IDS, emptySet()))
        }
        ...
        for (summary in summaries) {
            val id = summary.id?.toString() ?: continue

            if (endurainLoggedIn && !enduDone.contains(id)) {
                val (ok, reason) = uploadEndurainBlocking(context, gbDevice, summary)
                if (ok) {
                    enduDone.add(id)
                } else {
                    endurainFailure = reason
                    LOG.warn("Auto-upload to Endurain failed for summary {}: {}", id, reason)
                }
            }
            ...
            if (wandererLoggedIn && !wandDone.contains(id)) {
                ...
                if (gpx != null) {
                    val (ok, reason) = WorkoutUploader.uploadToWandererBlocking(context, gpx)
                    if (ok) {
                        wandDone.add(id)
                    } else {
                        wandererFailure = reason
                        LOG.warn("Auto-upload to Wanderer failed for summary {}: {}", id, reason)
                    }
                }
            }
        }

        // Persist the updated "already uploaded" sets (copies, never the returned instances).
        GBApplication.getPrefs().preferences.edit()
            .putStringSet(GBPrefs.ENDURAIN_UPLOADED_IDS, HashSet(enduDone))
            .putStringSet(GBPrefs.WANDERER_UPLOADED_IDS, HashSet(wandDone))
            .apply()
    ...
    private fun uploadEndurainBlocking(
        ...
        val fit: File = try {
            ...
            return false to e.localizedMessage
        }
