...
//        LOG.info("ATTRIBUTE:{} value:{}/{} length:{}", entry.getKey(), new String(bytes), GB.hexdump(bytes), bytes.length);
    ...
    public NotificationUpdateMessage onNotification(NotificationSpec notificationSpec) {
        ...
        return new NotificationUpdateMessage(
                ...
                hasActions,
                hasPicture,
                notificationSpec.frequencySuppressed
        );
    }
    ...
    public NotificationUpdateMessage onDeleteNotification(int id) {
        ...
        while (iterator.hasNext()) {
            ...
            if (e.getId() == id) {
                iterator.remove();
                return new NotificationUpdateMessage(NotificationUpdateMessage.NotificationUpdateType.REMOVE, e.type, getNotificationsCount(e.type), id, false, false, false);
            }
        }
        return null;
    }
