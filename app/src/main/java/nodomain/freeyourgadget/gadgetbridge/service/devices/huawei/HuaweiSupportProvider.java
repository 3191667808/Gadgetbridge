                        ...
                        try (DBHandler db = GBApplication.acquireDB()) {
                            ...
                            for (int i = 0; i < points.length; i++) {
                                ...
                                HuaweiWorkoutDataSample workoutSample = db.getDaoSession().getHuaweiWorkoutDataSampleDao().queryBuilder()
                                        .where(eq, HuaweiWorkoutDataSampleDao.Properties.WorkoutId.eq(fileRequest.getDatabaseId())).orderAsc(HuaweiWorkoutDataSampleDao.Properties.Timestamp).limit(1).unique();
