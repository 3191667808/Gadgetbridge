...
public class CalendarManager {
    ...
    private List<CalendarEvent> getCalendarEvents(final int lookaheadDays) {
        ...
        try (Cursor evtCursor = mContext.getContentResolver().query(eventsUri, null, selection.toString(), selectionArgs, Instances.BEGIN + " ASC")) {
    ...
    private boolean isExtendedPropertyBlacklisted(long eventId) {
        ...
        if ((!filterFocusTime && !filterWorkingLocations) || Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
            return false;
        }
